...
d['y'] = d['label'].apply(lambda x: np.array([np_utils.to_categorical(y,5) for y in tag[x].reshape((-1,1))]+[np.array([[0,0,0,0,1]])]*(maxlen-len(x))))
...
x = Embedding(len(chars)+1, word_size, input_length=maxlen, mask_zero=True)(inputs)
blstm = Bidirectional(LSTM(64, return_sequences=True), merge_mode='sum')(x)
